...
# raw read directory
rawread_path<-file.path(data_path,"rawreads1")
# filtered read directory
filt_path<-file.path(data_path,"filtreads1")
if(!file_test("-d",filt_path)) dir.create(filt_path)

# set temporary directories, if needed
tempfig_path<-file.path(figs_path,"Build_Phyloseq1")
if(!file_test("-d",tempfig_path)) dir.create(tempfig_path)
temprds_path<-file.path(rds_path,"Build_Phyloseq1")
...
plot.read.qual.f<-grid.arrange(
	...
	plot.qual.f[[5]],plot.qual.f[[6]],nrow=2,ncol=3)
ggsave(file.path(tempfig_path,"read.qual.f.pdf"),plot.read.qual.f,width=2*3,height=2*2,limitsize=FALSE)
...
plot.read.qual.r<-grid.arrange(
	...
	plot.qual.r[[5]],plot.qual.r[[6]],nrow=2,ncol=3)
ggsave(file.path(tempfig_path,"read.qual.r.pdf"),plot.read.qual.r,width=2*3,height=2*2,limitsize=FALSE)
...
cbind(
	basename(filtFs),
	sam.names,
	c("S23.1", "S23.2", "S23.3","S1.1", "S1.2", "S1.3")
)
sam.names<-c("S23.1", "S23.2", "S23.3","S1.1", "S1.2", "S1.3")
